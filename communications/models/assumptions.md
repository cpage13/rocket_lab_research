...
| `COMM-NEUTRON-ENVELOPE` | The baseline Neutron envelope carried by the model is 13.0 t to low-inclination LEO and 80 m3 usable fairing volume. Mirroring the data-center anchoring (`RLDC-FAIRING-VOLUME-80M3`), the Neutron Payload User's Guide v1.0 envelope is roughly 125 to 190 m3 usable at a 5.0 to 5.5 m fairing DIAMETER, so the 80 m3 dial is conservative, and for a large folded antenna the binding constraint is fairing DIAMETER, not volume or mass. | `scenario` | Model input, public doc claim | `inputs.config.launch.neutron_mass_envelope_t`; `inputs.config.launch.neutron_fairing_volume_m3`; `research/SOURCE_INDEX.md` `NTR-007` | The fairing-diameter binding and the real usable volume are under post-mortem review (2026-06-22); the corrected geometry is being re-sourced through the research-wiki before promotion. |
| `COMM-BLOCK-UPGRADE` | An upgraded-Neutron (Block-upgrade) scenario is carried as a configurable option, default OFF: 15.0 t mass envelope and 100 m3 fairing volume, versus the 13.0 t / 80 m3 baseline. This is the founder-stated assumption and mirrors the data-center block upgrade (`RLDC-PAYLOAD-SSO-UPGRADE`, 12.5 t SSO). | `scenario` | Model input, founder-stated assumption | `inputs.config.launch.upgraded_neutron` (false); `inputs.config.launch.upgraded_neutron_mass_envelope_t` (15.0); `inputs.config.launch.upgraded_neutron_fairing_volume_m3` (100.0); `research/SOURCE_INDEX.md` `NTR-007` | NOT a published Neutron guarantee. NOTE (post-mortem 2026-06-22): the upgrade adds MASS, but a large folded direct-to-cell antenna is fairing-DIAMETER-bound and the upgrade does not widen the fairing, so it does not raise the direct-to-cell satellites-per-launch count; it does help a mass-bound broadband satellite. |
