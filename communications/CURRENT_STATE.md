...
- The active model code lives in `code/src/communications/`: four modules
  (constants, config, engine, ground) carrying both model families.
- The test suite is green post-cleanup: the frozen Iridium suite, the
  ...
  data-center parity gate (174 tests at build close, mypy strict and ruff
  clean).
...
zero numeric discrepancies and four citation ids corrected in code.
...
- The cash cost per subscriber (7.95 dollars per year) is a build-year
  artifact; aligning to the annualized basis (about 17 dollars per subscriber
  per year at 10 million) is a tracked open item.
- The old pre-rewrite communications tree (the former CLI, output, comparison,
  ...
  is the four-module engine plus its per-family tests.
...
- Catalog reconciliation: fold the `COMM-561` to `COMM-710` claim blocks into
  `research/SOURCE_INDEX.md` and the catalogs.
