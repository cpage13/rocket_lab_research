...
| [direct_communication/spectrum_and_phased_array_fundamentals.md](direct_communication/spectrum_and_phased_array_fundamentals.md) | The first-principles spectrum and phased-array explainer (comms wave 8, reference, no verdict): what radio spectrum is, how a frequency carries data (modulation, the Shannon limit, carrier aggregation vs OFDMA, frequency reuse, allocation), and how phased-array antennas work (gain and aperture, the link budget, electronic beam steering, the three binding limits). The companion the band-designations reference fills the letter-band gap for. | Keeps frequency (band, sets reach), bandwidth (channel width, sets the capacity ceiling), and data rate (bandwidth x efficiency) strictly apart, the root confusion of the whole comms corpus. Separate channels ADD (carrier aggregation) while sub-carriers only SHARE (OFDMA); frequency reuse across non-overlapping beams multiplies total capacity. A satellite reaches a bare phone only because the satellite supplies all the gain (the ~0.2 W handset is ~25 dB short at LEO), which is why direct-to-cell carries a giant aperture; a phased array steers hundreds of spectrum-reusing beams electronically, bound in order by the beamforming chip, DC power, then held spectrum (spectrum binds first on a thin cellular holding). Owns COMM-561..574 (COMM-575..600 reserved-unused). |
...
| [node_design/solar_radiator_cost_refresh_2026_07.md](node_design/solar_radiator_cost_refresh_2026_07.md) | July 2026 refresh of both cost dials against live market pricing, the AI-1 reveal, and a bottom-up radiator BOM. | The investor-set `$20k/kW` pair reads central-to-conservative: solar implies about `$15/W` at the array (bracketed by live space-silicon prices), the productized radiator BOM lands `$3-9k/kW` booked (`$8-16k/kW` with the area win stripped); evidence class is estimate/BOM, not vendor quotes. |
