...
**Not just a render [FACT/INFERENCE].** Flatellite is a real production platform with a reported backlog exceeding 40 spacecraft, validating the volume-manufacturing intent as a real program, not a concept. **Correction (bus-naming, COMM-269):** the $816M SDA Tracking Layer Tranche 3 award (18 missile-tracking spacecraft, Dec 2025) is built on Rocket Lab's **Lightning** bus, NOT Flatellite, so it should not be cited as "Flatellite production." Rocket Lab's comms SDA award is the separate $515M Transport Layer-Beta Tranche 2 ([SpaceNews: SDA confirms Rocket Lab to produce 18 satellites](https://spacenews.com/space-development-agency-confirms-rocket-lab-will-produce-18-satellites-for-u-s-military-network/), Jan 2024 award), which predates Flatellite (Feb 2025) and is built on the earlier Photon/Pioneer-class bus, not Flatellite. [INFERENCE] The funded multi-spacecraft prime work still validates the production capability; it is just not Flatellite-specific.
...
- **Flatellite production rate.** Rocket Lab says "high volumes" and "mass manufacture" but has not published a satellites-per-month rate or a per-unit build time. The 40+ spacecraft backlog validates it as a real program, not a stated throughput. (Bus-naming, COMM-269: the $816M SDA contract's 18 spacecraft are missile-tracking Lightning-bus satellites, not Flatellite, so they are not "Flatellite production.")
