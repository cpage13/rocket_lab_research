...
**Baseline budget: the 12.5 t block-upgrade.** The project's operative mass budget is the **12.5 t block-upgrade SSO** baseline (`RLDC-PAYLOAD-SSO-UPGRADE`, the figure `data_center/assumptions.md` owns). The mass build-up in this document was originally framed against the more conservative **~9.5 t reusable-to-SSO** recovery case, so read the budgets and margins below (and the "1 rack per node" conclusion in the verdict and in §6 to §7) as the **conservative bracket**, not the baseline. Both are estimates: Rocket Lab publishes only LEO performance, so 12.5 t (upgraded Neutron) and 9.5 t (current reusable) are project assumptions, not certified SSO figures. Against the 12.5 t baseline a single rack's worth of node (~7 to 9 t) fits with comfortable room, and the promoted model spends that room on more compute: it packs per-package units (not whole racks) until the node nears 12.5 t, on the order of two racks' worth in 2026. So the operative node is heavier, and carries more compute, than the single-rack floor analyzed here.
...
**Relationship to the promoted model.** This doc builds the node mass bottom-up and itemized: a whole GB300 rack plus an explicit bus, propulsion, and structure, landing a 1-rack node at ~7-9 t against the conservative ~9.5 t reusable-to-SSO budget. The promoted model (`code/`) decomposes the same node differently. It sizes against the 12.5 t block-upgrade envelope and splits the node mass into a single fixed bus dial (`node_mass_fixed_t`, ~2.5 t) plus N compute "packages," where each package carries its own GPU/compute mass and its apportioned share of solar and radiator. So the comparison is direct: this doc's bus + propulsion + structure maps to the model's ~2.5 t fixed dial; this doc's node-level solar and radiator are folded into the model's per-package mass instead of broken out; and this doc's rack count maps to the model's package count. A package is roughly 1/72 of an NVL72 rack, so the model's ~146 packages in 2026 is about two racks and its ~37 in 2036 is about half a rack (package count falls over time because per-package mass rises with GPU power). It is the same physical node viewed two ways: this is the itemized whole-rack cross-check, and the model is the envelope-utilization view that floats package count to fill the mass budget each year.

**Why the promoted model's per-package arrays come out lighter, and where it stays conservative.** This document sizes solar and radiator from today's flown, off-the-shelf hardware, which is why its bottom-up is the heavier, conservative bracket. The promoted model assumes lighter, purpose-built arrays, and that assumption is grounded rather than a wish: (1) the node is sized for a **~5 to 7 year service life** (see §3), not the 20 to 30 year life a GEO array carries, so it does not haul decades of radiation and end-of-life degradation margin; (2) the **dawn-dusk sun-synchronous orbit** holds a stable geometry that lets the solar and radiator co-mount, share structure, and skip heavy sun-tracking gimbals (§4); and (3) Rocket Lab builds these in house and iterates them across cadence, the vertical-integration thesis applied to the hardware. The lighter mass is an engineered target justified by purpose-built, short-life design, not flown heritage. The project treats it as a central target with documented upside and downside bands, and stays conservative elsewhere (the cost multiples, no space-optimization credit, and this heavier off-the-shelf build-up kept on file as the floor).
...
**Service-life assumption (sizing the array light is realistic, not optimistic).** These arrays are sized for the node's **~5 to 7 year service life**, matching the node, not the 20 to 30 year life a GEO comsat array is built to survive. A short-life array does not need the thick coverglass, heavy radiation shielding, or large end-of-life degradation oversizing that decades on orbit demand, so a lighter modern silicon or hybrid array is a realistic choice here rather than an aggressive one. The lighter specific-power figures above should be read in that light: the weight is low because the required life is short, not because the array is assumed to beat physics.
